...
Because the BMA procedure is unusual, I replicate these results using a standard logistic regression analysis. For each outcome variable, I estimate a logistic regression model including all the covariates as explanatory variables and compute the effect of moving from a single-member district to a ten-member district for each respondent in the data set and average across all respondents, as suggested by \cite{HanmerKalkan2013}. I use Clarify-like simulation to obtain a 90\% confidence interval around this estimate \citep{KingTomzWittenberg2000}. In order to provide strong support for the hypotheses, the 90\% confidence intervals should only contain values that are consistent with a negligible effect \citep{Rainey2014}. Figure \ref{fig:logit} provides the estimated effect of shifting district magnitude from one to ten and the estimated confidence intervals. But which effects are substantively meaningful and which effects are negligible? \cite{BlaisAarts2006} provides an excellent summary of prior estimates of the effect of district magnitude. They provide seven estimates from four studies that argue for a positive effect of district magnitude on turnout. The average of these estimates is about ten and the minimum is about five. In arguing that the United States switch to proportional representation, \cite{Lijphardt1997} suggests that effect would be a nine to twelve percentage point increase in turnout. Thus any effect smaller than 0.10 casts some doubt on prior estimates and any estimate smaller than 0.05 is not substantively important. Also, keep in mind that I included all the variables used in the BMA procedure in this single logistic regression. To the extent that some of these variables are unrelated to the outcomes, the confidence interval increases. 

In spite of emphasizing unbiasedness at the expense of variability, the data still strongly support the claim that district magnitude has little to no effect on turning out to vote. The upper bound of the confidence interval is about 0.40, suggesting that the effect of district magnitude is much smaller than suggested by previous research and perhaps even substantively negligible. None of the other estimates are statistically significant, but the data do not rule out meaningful effects in these cases. However, it is worth pointing out that these data do not offer evidence \textit{against} my claim that district magnitude has no effect on the mediating outcomes. Instead, the evidence is ambiguous, consistent with both no effect and small to moderate effects.  
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale = 1]{figs/logit.pdf}
